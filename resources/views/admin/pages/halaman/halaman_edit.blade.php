...
@section('content')
    @if(!empty($halaman))
    @php  $halaman_id = $halaman->id;
    $judul = Input::old('judul')!==null?Input::old('judul'):$halaman->judul;
    $slug = (Input::old('slug')!==null)?Input::old('slug'):$halaman->slug;
    $konten = (Input::old('konten')!==null)?Input::old('konten'):$halaman->konten;
       if (count($halaman->Media)>0){
                    $alt_foto = (Input::old('alt_foto')!==null)?Input::old('alt_foto'):$halaman->Media[0]['title'];
                    $foto = config('desawisata.PATH_IMAGE_HALAMAN').$halaman->Media[0]['filename'];
       }else {
            $alt_foto = '';
            $foto ='images/profil_desawisata.png';
       }

    $status = Input::old('status')!==null?Input::old('status'):$halaman->status;
@endphp
...
<div class="dashboard-content">
    <div class="dashboard-inner">
        <div class="container-fluid">
            ...
            <form class="form-custom has-category" action="{{ route('halaman.update',$halaman) }}" method="POST" id="form-halaman"
                    ...
                    <div class="form-based">
                        ...
                        <div class="form-group has-two-fields">
                            <div class="form-category-entry">
                                <div class="category-field">
                                <input type="hidden" name="id" value="{{ $halaman_id }}">
